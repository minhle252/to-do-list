...
import { findToDoByName, removeTodoMultiple, togleFormTask } from "@/redux/todoList/toDoSlice";
...
export default function Home() {
  ...
  const toggleMultiple = ()=>{
    if(listId.length > 0){
      ...
      CONSTANTS.showToastFunc("You haven't selected the to do item yet!",'error');
    }
  }
      ...
      <div className="box_to_do_list flex flex-col flex-1 pt-5">
        <div className="flex-1 px-10">
          ...
          <input
            ...
            placeholder="Search..."
            defaultValue={""}
            onChange={(event:any) => dispatch(findToDoByName(event.target.value))}
            className="w-full px-2 py-1 "
          />
          <div className="overflow-auto h-[calc(95vh-236px)] mt-5">
            {todos?.map((item: any, index: number) => (
              item.showView ? <ToDoItemComponent key={index} selectedItem={listId.includes(item.id)} valueChecked={item.id} changeEvent={(event:any)=>changeListId(event) } item={item} /> : ''
            ))}
          </div>
        </div>
        <div className="bg-grays flex p-5 items-center border-t-2">
          <p className="flex-1">Bulk Action:</p>
          <button onClick={toggleMultiple} className="bg-blue text-white select-none py-1 px-5 rounded-md me-2">
            Detail
          </button>
          <button onClick={removeMultiple} className="bg-red text-white select-none py-1 px-5 rounded-md">
            Remove
          </button>
        </div>
      </div>
